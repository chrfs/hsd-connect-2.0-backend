...
const checkFileType = {
  image: (parsedFile: any) => {
    const acceptedTypes: string[] = ['image/jpeg', 'image/gif', 'image/webp', 'image/png']
...
export const saveFiles = {
  images: async (saveDir: string, imageArr: Image[]) => {
    try {
      if (!fs.existsSync(saveDir)) await Promise.resolve(mkdirp.sync(saveDir) as any)

      return Promise.all(
        imageArr.map((image: Image): any => {
