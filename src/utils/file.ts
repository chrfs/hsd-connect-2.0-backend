...
export const parse = {
  ...
  fileName: (fileName: string, ext: string, len = 5) => {
    let newFileName = fileName
      ...
      .replace(/[^a-z0-9]|\s+|\r?\n|\r/gim, "");
    newFileName =
      String(newFileName).length >= len
        ? newFileName.substr(0, 5)
        : Math.random()
            .toString(36)
            .substring(len);
  ...
  processImages: async (parsedImages: Image[], maxWidth: number) => {
    const compressableTypes = ["image/jpeg", "image/png"];
    const images = parsedImages.reduce(
      (acc, image, index) => {
        image.orderNo = index;
    ...
    images.inCompressable = process.inCompressableImages(
      images.saveDir,
      images.inCompressable
    );
    images.compressable = await process.compressableImages(
      images.saveDir,
      images.compressable,
      maxWidth
    );
    return {
      ...
      files: images.compressable
        .concat(images.inCompressable)
        .sort((img1, img2) => img1.orderNo - img2.orderNo)
    };
  }
};

const process = {
  compressableImages: async (
    imageDir: string,
    imageArr: Image[],
    maxWidth: number
  ) => {
    ...
    return Promise.all(
      jimpImages.map(async (image, imageIndex) => {
        const newImage: any = {
        ...
        newImage.data =
          image.bitmap.width >= maxWidth
            ? await image.resize(maxWidth, Jimp.AUTO).quality(80)
  ...
  images: async (saveDir: string, imageArr: Image[]) => {
    try {
      if (!fs.existsSync(saveDir)) {
        await Promise.resolve(mkdirp.sync(saveDir) as any);
      }
        ...
        imageArr.map(
          (image: Image): any => {
            ...
            if (!fs.existsSync(imagePath)) {
              return image.data instanceof Jimp
                ? image.data.writeAsync(imagePath)
