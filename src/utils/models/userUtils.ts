...
export const userValidator = {
  validateEmail(next: any) {
    ...
    const isValid =
      ...
      suffix.some((su: string) =>
        new RegExp(`${su}$`).test(this.email.toLowerCase().replace(/\s/g, ""))
      );
    if (!isValid) throw userValidationErrors.invalidEmail;
    this.email = this.email.toLowerCase();
    return next();
  },
  validatePassword(next: any) {
    try {
...
export const userRecordUtils = {
  ...
  async setHashedPassword(next: any) {
    try {
      if (!this.isNew && !this.isModified("password")) return next();
