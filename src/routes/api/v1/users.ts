...
const authenticateUser = function (password: string, user: UserNamespace.UserInterface) {
  try {
    if (!user) {
      return false
    }
...
const createUser = (userProperties: any): Promise<UserNamespace.UserInterface> => {
  try {
    return new User({
...
router.put('/:userId/bookmark/:projectId', async (ctx: any) => {
  try {
    ...
    if (user.bookmarkedProjects.some((bookmark: any) => bookmark.equals(project._id))) {
